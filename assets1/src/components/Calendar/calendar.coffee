define (require, exports, module)->
    ...
    class Calendar extends CalendarBase
        ...
        * @param dateText string 时间文本
        ...
        formatDate: (dateText)->
            ...
            switch
                when pattern.year.test(dateText) then new Date(parseInt(dateText, 10), 0, 1) # 一月->0  -2018-6-5--122
        ...
        * @param value 值, 可为日期对象或文本
        ...
        isDisabled: (value, type) ->
            ...
            if type is "month" and @options.week and not valueDisabled
                week = @options.week[date.getFullYear() + '-' + (value.getMonth() + 1)]
                valueDisabled = !(week and week.length)
        ...
        * @param value 值 , 可为日期对象或文本
        ...
        isSelected: (value) ->
            ...
            return (value + '' in @select.map (v) -> v + '')
        ...
        formatData : (date) ->
            ...
            for i in [0...12]
                ...
                monthData[j].push
                    ...
                    value: dataValue
                    disabled: @isDisabled(@formatDate(dataValue), "month")  # 周度过滤问题  -2018-6-5--122
    ...
    class Calendar.decorators.Day extends Calendar
        ...
        formatTitle : (select, index)->
            ...
            if $.isArray(select)
                ...
                selectLzz = select[0]
                if (typeof selectLzz is 'string')
                    selectLzz = selectLzz.replace(/-/g, '/').split('/');
                    selectLzz[1] = selectLzz[1] or '1'
                    selectLzz[2] = selectLzz[2] or '1'
                    selectLzz = selectLzz.join('/')

                selectTitle = @stringify(selectLzz, 'month')
                if (new Date(selectLzz)).getMonth() isnt @add(@renderDate, index, 'month').getMonth()
                    selectTitle = @stringify(@add(new Date(selectLzz), 1, 'month'), 'month')
    ...
    $.fn.extend
        ...
        calendar: (options={}) ->
            ...
            $(document).on 'click.toggleCalendar, focus.toggleCalendar', (event)->
                ...
                if $target.is($input)
                    ...
                    $uiPopup.hide()
                    $uiPopup.find('.ui-year').hide()    # 年不收起  -2018-6-5--122
        ...
        monthCalendar: (options={}) ->
            ...
            $calendar.calendar(options).off('dateChange').on 'dateChange', (event, value) -> $(this).trigger('change', [value])
        ...
        gfCalendar: (options={}) ->
            ...
            this.each ->
                ...
                $gfc = $(this).addClass('ways-popup-calendar gf-popup-calendar')
                ...
                $gfc.off('submit')
                    ...
                    tab = newTab = options.tab or 0

                    if options.select is 'today'
                        options.select = Calendar.prototype.stringify(new Date(), 'month')

                    $gfc.append("<input type='text' class='text-field waysdatepicker' value='#{if tab is 0 then options.select or '' else i18n.month(options.select) or ''}' />")

                    $uiPopup  = $("<div class='ways-calendar ui-popup' style='display: none; position: absolute; z-index: 1024;'/>").appendTo($gfc)

                    $uiPopupContent = $("""
                        <div class="ui-popup-content">
                            <div class="tabs" style="padding-top: 0px !important;">
                                <ul class="nav nav-tabs">
                                    <li><a data-toggle="tab">#{lang.singleMon}</a></li>
                                    <li><a data-toggle="tab">#{lang.singleMonOfYear}</a></li>
                                </ul>
                    ...
                    $input = $gfc.find('input');

                    $gfc.on 'submit', ->
                        $uiPopup.hide()

                    $gfc.find('input').click ->
                        $tabs.find(".nav-tabs li:eq(#{tab})").click()

                    calendars = {}

                    do ->
                        $uiCalendar = $("<div class='ui-calendar'/>")
                        $uiCalendar.appendTo($uiPopupContent.find('.tab-pane:eq(0)'))
                        cOptions = $.extend true, {}, {
                            element: $gfc
                            type: 'Month'
                            input: $input
                            container: $uiCalendar
                            itemPerLine: 6
                            itemPerPage: 12
                            mode: 'radio'
                        }, options, {select: if tab is 0 then options.select else ''}
                        type = util.firstLetterToUpper(cOptions.type)
                        calendar = new Calendar.decorators[type](cOptions)
                        ...
                        $uiPopup.mouseenter ->
                            $(document).off('click.toggleCalendar, focus.toggleCalendar')

                        $uiPopup.mouseleave ->
                            $(document).on 'click.toggleCalendar, focus.toggleCalendar', (event)->
                                $uiPopup.find('.ui-calendar>table').hide()
                                $uiPopup.hide()
                                $(document).off('click.toggleCalendar, focus.toggleCalendar')

                        calendars.c0 = calendar

                    do ->
                        $uiCalendar = $("<div class='ui-calendar'/>")
                        $uiCalendar.appendTo($uiPopupContent.find('.tab-pane:eq(1)'))
                        ...
                        cOptions = $.extend true, {}, {
                            ...
                            formatText: (select)->
                                ...
                                if $.isArray(select)
                                    selectTitle = i18n.month(select[0].split('-')[1])
                                    selectTitle += "~#{i18n.month(select[select.length - 1].split('-')[1])}" if select.length > 1

                                selectTitle
                        }, options, {
                            select: if tab is 1 then "#{newYear}-#{options.select}" else ''
                            min: "#{newYear}-1",
                            max: "#{newYear}-12"
                        }
                        type = util.firstLetterToUpper(cOptions.type)
                        ...
                        $uiCalendar.find('.ui-calendar-header').closest('thead').hide()
                    ...
                    $tabs = $uiPopup.find(".tabs")

                    oldValue = options.select

                    $gfc.data('tab', tab or 0)
                    $gfc.data('select', oldValue or '')

                    $tabs.change (event, value)->
                        newTab = value
                        event.stopPropagation()

                    $gfc.on 'submit', (event, value)->
                        if (tab isnt newTab)
                            tab = newTab
                            value = value.split('-')[1] if newTab is 1
                            $gfc.trigger('change', [value, newTab]) if value is oldValue
                            $gfc.data('tab', newTab)
                            $gfc.data('select', value)
                        for key, calendar of calendars when key isnt "c#{tab}"
                            calendar.ui.find('.selected').removeClass('selected')
                        oldValue = value
                        event.stopPropagation()

                    $gfc.off('dateChange').on 'dateChange', (event, value)->
                        value = value.split('-')[1] if newTab is 1
                        $gfc.trigger 'selectDate', [value, newTab]
                        $gfc.trigger 'change', [value, newTab]
                        $gfc.data('select', value)
        ...
        edaCalendar: (options={}) ->
            ...
            this.each ->
                ...
                $gfc = $el.addClass('ways-popup-calendar eda-popup-calendar')
                ...
                originOptions = null
                    ...
                    $gfc.append("<input type='text' class='text-field waysdatepicker' value='#{options.select}-#{selectWeek}' />") unless reset

                    $uiPopup  = $("<div class='ways-calendar ui-popup' style='display: none; position: absolute; z-index: 1024;'/>").appendTo($gfc)

                    $uiPopupContent = $("""
                        <div class="ui-popup-content">
                            <div class="date-pane"></div>
                            <hr style="margin: 0 5px; border: 0; border-top: 1px solid #C5DBEC;"/>
                            <table cellpadding="0" cellspacing="0" class="ui-month ui-week">
                                <tbody class="ui-selectors">
                                    <tr class="ui-calendar-content">
                                        <td class="ui-container" colspan="3">
                                            <div class="slide-box">
                            ...
                            <div class="ui-button-container">
                                <button class="ui-button ui-reset">重置</button>
                                <button class="ui-button ui-confirm">确定</button>
                            </div>
                        </div>
                    """).appendTo($uiPopup)

                    week = options.week[options.select]
                    weekTds = _.map [1..5], (item)->
                        className = [
                            if selectWeek is "W#{item}" then 'selected' else ''
                            if _.includes(week, item) then '' else 'disabled'
                        ].join(' ')
                        """<td><a data-value="W#{item}" class="#{className}" title="W#{item}">W#{item}</a></td>"""
                    .join('')
                    $uiPopupContent.find('.week-container').append(weekTds)

                    $input = $gfc.find('input')

                    $gfc.data('select', options.select or '')

                    $gfc.off('submit').on 'submit', ->
                        $uiPopup.hide()

                    do ->
                        $uiCalendar = $("<div class='ui-calendar'/>")
                        $uiCalendar.appendTo($uiPopupContent.find('.date-pane'))
                        cOptions = $.extend true, {}, {
                            element: $gfc
                            type: 'Month'
                            input: $input
                            ...
                            onConfirm: (selectDate)->
                                # options.select = selectDate
                                $gfc.data('select', selectDate)
                                return false if not selectWeek or not clickedWeek
                            onShowMonths: ->
                                unless $uiPopup.find('.ui-week .ui-container a:not(.disabled)').length
                                    $uiPopup.find('.ui-month:not(.ui-week) .ui-container a:not(.disabled).selected').click()
                    ...
                    # 选周
                    $uiPopup.on 'click', ".ui-week a:not(.disabled)", ->
                        # clickedWeek = true
                        selectWeek = $(this).data('value')
                        $(this).addClass('selected').closest('td').siblings('td').find('a.selected').removeClass('selected')
                        # $uiPopup.find('.ui-month a.selected').mousedown()
                        # clickedWeek = false

                    # 日期变更时广播相关事件
                    $gfc.off('dateChange').on 'dateChange', (event, value)->
                        value = "#{value}-#{selectWeek}"
                        $gfc.trigger 'selectDate', [value]
                        $gfc.trigger 'change', [value]
                        $gfc.data('select', value)

                    # 选月
                    $uiPopup.on 'click', '.ui-month:not(.ui-week) .ui-container a:not(.disabled)', ->
                        week = options.week[$gfc.data('select')]
                        $weekElms = $uiPopup.find(".ui-week .ui-container a")
                        _.map [1..5], (item)->
                            $week = $weekElms.eq(item - 1)
                            unless _.includes(week, item)
                                $week.removeClass('selected').toggleClass 'disabled', true
                            else
                                $week.toggleClass 'disabled', false
                        $weekElms.filter(':not(.disabled):last').click()

                    # 切换到年面板时禁用周选项
                    $uiPopup.on 'click', '.ui-current', ->
                        $weekElms = $uiPopup.find(".ui-week .ui-container a")
                        $weekElms.removeClass('selected').addClass 'disabled'

                    # 左右切换整年月份
                    $uiPopup.on 'click', '.ui-nav', ->
                        return if $uiPopup.find('.ui-month').is(':not(:visible)')
                        setTimeout ->
                            $selected = $uiPopup.find('.ui-month:not(.ui-week) .ui-container a:not(.disabled).selected')
                            if $selected.length
                                $selected.click()
                            else
                                $uiPopup.find(".ui-week .ui-container a").removeClass('selected').addClass 'disabled'
                        , 250

                    # 重置还原默认时间
                    $uiPopup.on 'click', '.ui-button-container .ui-reset', ->
                        $uiPopup.remove()
                        $uiPopup.off()
                        $(document).off('click.toggleCalendar, focus.toggleCalendar')
                        options = $.extend true, {}, originOptions
                        init(options, true)
                        $el.find('input').click()

                    # 确定
                    $uiPopup.on 'click', '.ui-button-container .ui-confirm', ->
                        clickedWeek = true
                        $uiPopup.find('.ui-month a.selected').mousedown()
                        clickedWeek = false

                init = (options, reset)->
                    selectWeek = options.select.split('-')[2]
                    options.select = options.select.replace(/-W\d$/, '')
                    done(reset)
                    options.onRender?(options)

                url = options.url or options.ajax && options.ajax.url
                if url
                    $.ajax _.extend({
                        url: url,
                        async: false,
                        ...
                        success: (dateJson) ->
                            $.extend options, data, dateJson
                            originOptions = $.extend true, {}, options
                            init(options)
                    }, options.ajax)
                else
                    originOptions = $.extend true, {}, options
