...
class Table extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      ...
      const { page, pageSize: statePageSize } = this.state;
  ...
  componentWillUpdate(nextProps, { onExpandRowKeysChange, expandedRowKeys }) {
    if (onExpandRowKeysChange && expandedRowKeys !== this.state.expandedRowKeys) {
      onExpandRowKeysChange(expandedRowKeys);
    }
  }
  ...
  filterOnlyOneExpandedRow(expandedRowKeys, expanded, record, expandedDepth) {
    ...
    if (expanded) {
      const sameLevelExpandedRows = [
        ...
        ..._.flatten(dataSource.filter(row => row.children)
