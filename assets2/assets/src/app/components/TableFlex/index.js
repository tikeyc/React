...
class Table extends Component {
  ...
  getVisibleChildrenColumns(columns) {
    return _.flatten(columns.filter(column => column.children).map(column => column.children));
  }
      ...
      let result = [
        ...
        >
          {columns.map(({ key, render, className, style, drillPoint }, colIndex) => {
            ...
            const tdStyle = typeof row.tdStyle === 'object' ? { ...row.tdStyle, ...style } : style;
  ...
  freeze = options => {
    ...
    const visibleColumns = this.getVisibleTbodyColumns(columns);    // 每一列 对应 一对像
    if (visibleColumns.length === 0) return;

    let tableWidth = 0;                     //
    const widths = [];                      //
    const $table = $(this.refDataTable);    // 主体 》table
    const cols = 'colgroup col';
    const $tds = $table.find('tbody tr:first td');  // 主体 》tbody tr:first ad Arr
    const $cols = $table.find(cols);                // 主体 》col Arr
    const $fixedTableHeader = $(this.refFixedTableHeader);  // 悬浮头 》table
    const $fixedHeaderThs = $fixedTableHeader.find(cols);   // 悬浮头 》col Arr
    const $fixedAsideThs = $(this.refFixedTableAside).find(cols);   // 侧边栏 》col Arr
    const $fixedHeaderAsideThs = $(this.refFixedTableHeaderAside).find(cols); // 侧边栏头 》col Arr
    const scrollbarWidth = $(this.refTableWrapper).find('.scroll-y').outerWidth();  // 最外层div 下 Y滚动轴的宽度
    ...
    // 股东里面的table高度  <-->  最外层div高度
    const hasScrollY = $table.outerHeight() > $(this.refTableWrapper).outerHeight();
    /** D
    $fixedTableHeader.parent().css({ 'margin-right': hasScrollY ? scrollbarWidth : 0 });  // zzZ
    // */
    ...
    // 给 table 元素设置实际的总宽度  [:生成的table 宽度 是否大于 外层div的宽度]
    /** D
    if (tableWidth > this.tableWidth) {
    // */
    if (tableWidth > $(this.refTableWrapper).width()) {
  ...
  render() {
    ...
    return (
      ...
      >
        {/* **************************** 主体 **************************** */}
        <div className="table-scroller scrollbar-inner" ref={ref => { this.refDataTableScroller = ref; }}>
          <table className="table" ref={ref => { this.refDataTable = ref; }}>
            {header}
            <tbody>
