...
export default class DropdownMenu extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return (
      ...
      this.props.allowZero !== nextProps.allowZero
    );
  }
  ...
  selectMultipleOption(options, indexs, parent) {
    ...
    if (theParent) {
      theParent.selected = options.every(option => option.selected);
    }
  }
  ...
  handleSelect(value, text, indexs) {
    ...
    const { multiple, allowZero } = this.props;
    let nextOptions;
    if (multiple) {
      ...
      if (options.find(v => v.selected) && options.find(v => v.selected).value === value) {
  ...
  renderLi(option, indexs) {
    ...
    const { multiple, optionRender, allowZero } = this.props;
    let childrenList;
    ...
    return [
      value != null ? (
        <li
          ...
          onClick={() => {
            if (multiple || !selected || allowZero) this.handleSelect(value, text, indexs);
          }}
        >
          {!optionRender ? <a style={style}>{text}</a> : optionRender(option)}
        </li>
      ) : (
        <li
