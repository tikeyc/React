...
export default class MapChart extends Chart {
  getOptions() {
    ...
    if (series.length > 0) {
      series = series.map(item => {
        if (item.type === 'map') {
          const { mapType, mapLevel } = item;
          return {
            ...item,
            mapType: mapLevel === 'city' ? mapType.replace(/市$/, '') : mapType,
            mapSource: `${mapLevel}/${mapType}`,
          };
        }
        return item;
      });
  ...
  draw() {
    ...
    if (series) {
      const mapSeries = series.filter(({ type, mapType }) => type === 'map' && mapType !== 'china');
